...
import { Calendar as CalendarIcon, Clock, MapPin, ArrowRight, Video, Users, CheckCircle, Plus, BarChart3 } from 'lucide-react'
...
export function DepartmentHeadDashboard() {
  ...
  const [pastEvents, setPastEvents] = useState<(Event & { registrationCount: number; attendanceCount: number })[]>([])
  ...
  const fetchDashboardData = useCallback(async () => {
    ...
    try {
      const [recentAnnouncements, futureEvents, historyEvents] = await Promise.all([
        ...
        eventService.getPastEventsForDepartment(user.department, 5)
      ])
  ...
  const actions: QuickAction[] = [
    { label: 'Manage Roster', href: '/department/roster', icon: <CalendarIcon className="w-6 h-6" />, colorClass: 'bg-indigo-500/10 text-indigo-500' },
    { label: 'New Meeting', href: '/meetings/new', icon: <Video className="w-6 h-6" />, colorClass: 'bg-rose-500/10 text-rose-500' },
    { label: 'Message Dept', href: '/department/message', icon: <Plus className="w-6 h-6" />, colorClass: 'bg-emerald-500/10 text-emerald-500' },
    { label: 'Reports', href: '/department/reports', icon: <BarChart3 className="w-6 h-6" />, colorClass: 'bg-blue-500/10 text-blue-500' },
  ]

  const getScanRate = (evt: any) => {
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-8">
              ...
              <motion.div initial="hidden" animate="visible" variants={fadeUp} className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <StatsCard title="Total Workers" value="24" icon={<Users className="w-5 h-5 text-indigo-500" />} />
                <StatsCard title="On Duty Today" value="8" icon={<CheckCircle className="w-5 h-5 text-emerald-500" />} />
                <StatsCard title="Upcoming Meetings" value="2" icon={<Video className="w-5 h-5 text-rose-500" />} />
                <StatsCard title="Pending Requests" value="3" icon={<Plus className="w-5 h-5 text-amber-500" />} />
              </motion.div>
              ...
              <motion.div initial="hidden" animate="visible" variants={fadeUp} className="space-y-4">
                <div className="flex items-center justify-between">
                  <h2 className="text-xl font-bold tracking-tight">Past Department Events</h2>
                </div>
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            ...
                            <div className="flex items-center gap-2 text-xs text-muted-foreground">
                              <span className="flex items-center gap-1"><Users className="w-3 h-3" /> {event.registrationCount} reg</span>
                              <span className="flex items-center gap-1"><CheckCircle className="w-3 h-3" /> {event.attendanceCount} att</span>
                            </div>
                          </div>
                        </div>
                        <div className="sm:self-end">
                          <span className="px-3 py-1 bg-primary/10 text-primary text-xs font-bold rounded-full flex items-center gap-1">
                            <BarChart3 className="w-3 h-3" /> Scan Rate: {getScanRate(event)}
                          </span>
                        </div>
                      </div>
                    ))
