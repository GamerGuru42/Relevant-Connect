...
export function SyncIndicator() {
  ...
  // Track when we actually go offline
  useEffect(() => {
    if (!isOnline) {
  ...
  // Only show "Back online" when we transition from offline → online, once
  useEffect(() => {
    if (isOnline && !syncing && pendingCount === 0 && wasOfflineRef.current && !hasShownSuccessRef.current) {
  ...
  // Don't render anything when online with nothing pending and no success message
  if (isOnline && pendingCount === 0 && !showSuccess && !syncing) return null

  return (
    <AnimatePresence>
      {(!isOnline || syncing || showSuccess) && (
        <motion.div
